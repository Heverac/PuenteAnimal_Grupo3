...
fun AppNavigation(userSessionViewModel: UserSessionViewModel) {
    ...
    val animalesApadrinados = remember { mutableStateListOf<Animal>() }
    ...
    Scaffold(
        ...
        NavHost(
            ...
            composable(route = Screen.Animales.route) {
                AnimalesScreen(
                    navController = navController,
                    animalesApadrinados = animalesApadrinados
                )
            }
            ...
            composable("apadrinar") {
                ApadrinarScreen(
                    navController = navController,
                    onAgregarApadrinado = { animal, monto ->
                        if (animalesApadrinados.none { it.nombre == animal.nombre }) {
